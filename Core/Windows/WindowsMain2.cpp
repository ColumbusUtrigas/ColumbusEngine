...
//     const char* names[10];
//     SDL_Vulkan_GetInstanceExtensions(window, &sdl_count, names);
...
Columbus::Vector3 vertices[] = {
    { 0.0, -0.5, 0},
    { 0.5, 0.5, 0},
    { -0.5, 0.5, 0}
};

Columbus::Vector2 uvs[] = {
    {0.5, 1},
    {1, 0},
    {0, 0}
};
...
int main()
{
    ...
    auto vbuf = device->CreateBuffer(sizeof(vertices), vertices, Columbus::BufferType::Array);
    auto ubuf = device->CreateBuffer(sizeof(uvs), uvs, Columbus::BufferType::Array);
    auto ibuf = device->CreateBuffer(sizeof(indices), indices, Columbus::BufferType::Index);
    ...
    auto descriptorSet = device->CreateDescriptorSet(static_cast<Columbus::Graphics::GraphicsPipelineVulkan*>(pipeline)->setLayout);
    ...
    while (running)
    {
        ...
        Columbus::BufferVulkan bufs[] = { vbuf, ubuf };
        uint64_t bufferOffsets[] = {0, 0};

        cmdBuf.BeginRenderPass(renderpass, rect, swapchain->swapChainFramebuffers[imageIndex], 1, &clearColor);
        cmdBuf.BindGraphicsPipeline(pipeline);
        cmdBuf.BindDescriptorSets(pipeline, 0, 1, &descriptorSet);
        cmdBuf.BindVertexBuffers(0, 2, bufs, bufferOffsets);
        cmdBuf.BindIndexBuffer(ibuf, 0);
        cmdBuf.DrawIndexed(3, 1, 0, 0, 0);
        cmdBuf.EndRenderPass();

        cmdBuf.End();

        device->Submit(cmdBuf, fence, imageSemaphore, submitSemaphore);
