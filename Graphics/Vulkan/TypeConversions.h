...
namespace Columbus
{
	...
	static VkFormat TextureFormatToVK(TextureFormat format)
	{
		switch (format)
		{
			...
			case TextureFormat::R8SRGB: return VK_FORMAT_R8_SRGB;
			case TextureFormat::RG8SRGB: return VK_FORMAT_R8G8_SRGB;
			case TextureFormat::RGB8SRGB: return VK_FORMAT_R8G8B8_SRGB;
	...
	{
		const TextureFormatInfo& FormatInfo = TextureFormatGetInfo(format);

		if (FormatInfo.HasDepth)
		{
			return FormatInfo.HasStencil ? (VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT) : VK_IMAGE_ASPECT_DEPTH_BIT;
		}
		else
		{
