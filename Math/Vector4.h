...
namespace Columbus
{
	...
	struct Vector4_t
	{
		...
		Vector4_t(Vector4_t&& Base) : X(Base.X), Y(Base.Y), Z(Base.Z), W(Base.W) {}
		...
		Type Distance(const Vector4_t& Other) const
		{
			return sqrtf(Math::Sqr(Other.X - X) + Math::Sqr(Other.Y - Y) + Math::Sqr(Other.Z - Z) + Math::Sqr(Other.W - W));
		}

		Type DistanceSquare(const Vector4_t& Other) const
		{
			return Math::Sqr(Other.X - X) + Math::Sqr(Other.Y - Y) + Math::Sqr(Other.Z - Z) + Math::Sqr(Other.W - W);
		}
...
CREFLECT_DECLARE_STRUCT_NATIVE(Columbus::Vector4, 1, "22CFF048-AD10-404B-AFB3-101261A3009E");
CREFLECT_DECLARE_STRUCT_NATIVE(Columbus::iVector4, 1, "CD0E653B-2E3D-4EC1-B74A-4012FC864651");
